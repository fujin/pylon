...
describe Pylon::Daemon do
  ...
  describe ".pid_file" do
    ...
    describe "without the pid_file option set" do
      ...
      before do
        Pylon::Config[:pid_file] = nil
        Pylon::Daemon.name = "chef-client"
      end
  ...
  describe ".change_privilege" do
    ...
    describe "when the user and group options are supplied" do
      ...
      before do
        Pylon::Config[:group] = 'staff'
      end

      it "should log an appropriate info message" do
        Pylon::Log.should_receive(:info).with("About to change privilege to aj:staff")
        Pylon::Daemon.change_privilege
      end

      it "should call _change_privilege with the user and group" do
        Pylon::Daemon.should_receive(:_change_privilege).with("aj", "staff")
        Pylon::Daemon.change_privilege
      end
    end
    ...
    describe "when just the user option is supplied" do
      before do
        Pylon::Config[:group] = nil
      end

      it "should log an appropriate info message" do
        Pylon::Log.should_receive(:info).with("About to change privilege to aj")
        Pylon::Daemon.change_privilege
      end

      it "should call _change_privilege with just the user" do
        Pylon::Daemon.should_receive(:_change_privilege).with("aj")
        Pylon::Daemon.change_privilege
      end
    end
  end
  ...
  describe "._change_privilege" do
    ...
    describe "with sufficient privileges" do
      ...
      it "should initialize the supplemental group list" do
        ...
        Pylon::Daemon._change_privilege("aj")
      end
    ...
    describe "with insufficient privileges" do
      ...
      it "should log an appropriate error message and fail miserably" do
        ...
        Pylon::Application.should_receive(:fatal!).with("Permission denied when trying to change 999:999 to 501:20. #{error}")
        Pylon::Daemon._change_privilege("aj")
      end
    end
